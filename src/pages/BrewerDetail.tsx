...
const BrewerDetail: React.FC = () => {
    ...
    const [relatedBrewers, setRelatedBrewers] = useState<Brewer[]>([]);
    ...
    useEffect(() => {
        if (brewer && userInfo?.id) {
            let imageUrl = '';
            if (brewer.images && brewer.images.length > 0) {
                imageUrl = brewer.images[0];
            } else if (brewer.driveImages && brewer.driveImages.length > 0) {
                imageUrl = `https://lh3.googleusercontent.com/d/${brewer.driveImages[0].fileId}?authuser=server`;
            }
    ...
    const getBrewerById = async () => {
        try {
            setLoading(true);
            if (id) {
                const brewerData = await brewerService.getById(id);
                setBrewer(brewerData);
            }
        } catch (error) {
            console.error('Error loading brewer:', error);
        } finally {
            setLoading(false);
        }
    };
    ...
    const getImages = (): string[] => {
        ...
        // Add regular images
        if (brewer.images && brewer.images.length > 0) {
            images.push(...brewer.images);
        }
        
        // Add drive images
        if (brewer.driveImages && brewer.driveImages.length > 0) {
            brewer.driveImages.forEach(img => {
    ...
    const getRelatedBrewers = async () => {
        try {
            const allBrewers = await brewerService.getAll();
            // Filter out current brewer and get random 4
            const filtered = allBrewers.filter(b => b.id !== id);
            const shuffled = filtered.sort(() => 0.5 - Math.random());
            setRelatedBrewers(shuffled.slice(0, 4));
        } catch (error) {
            console.error('Error loading related brewers:', error);
        }
    };
    ...
    if (!brewer) {
        return (
            <div className="flex justify-center items-center h-screen">
                <div className="text-lg text-gray-500">Không tìm thấy máy pha cà phê</div>
            </div>
        );
    }
                ...
                >
                    ...
                    <div className="space-y-1 bg-8am-white">
                        ...
                        <div className="mt-6 pl-4 pr-4 pb-10">
                            ...
                            {relatedBrewers.length === 0 ? (
                                <div className="text-center py-4">
                                    <div className="text-gray-500">Đang tải sản phẩm liên quan...</div>
                                </div>
                            ) : (
                                <div className="flex overflow-x-auto gap-4 pb-2">
                                    {relatedBrewers.map((relatedBrewer) => (
                                        <div key={relatedBrewer.id}>
                                            <MachineCard
                                                machine={relatedBrewer}
                                                type="brewer"
                                                isShowLike={false}
                                                width={230}
                                                userInfo={userInfo}

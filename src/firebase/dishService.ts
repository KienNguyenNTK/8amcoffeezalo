...
export class DishService {
  ...
  async getDishesFilteredByGroups(): Promise<Dish[]> {
    try {
      const appConfigs = await getDocs(collection(db, APP_CONFIGS));
      console.log('appConfigs', appConfigs.docs[0].data());

      const allowedGroupCodes = appConfigs.docs[0].data()?.groups_code;
      const querySnapshot = await getDocs(collection(db, COLLECTION_NAME));
      ...
      return dishes
        ...
        .sort((a, b) => {
          // So sánh theo thứ tự group trong allowedGroupCodes
          const groupAIndex = allowedGroupCodes.indexOf(a.group);
          const groupBIndex = allowedGroupCodes.indexOf(b.group);
          if (groupAIndex !== groupBIndex) {
            return groupAIndex - groupBIndex;
          }
          // Nếu cùng group, sort tiếp theo displayOrder hoặc code
          if (a.displayOrder !== undefined && b.displayOrder !== undefined) {
            return a.displayOrder - b.displayOrder;
          }
          if (a.code && b.code) {
